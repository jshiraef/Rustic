	...
	public sealed class Lua {
		...
		public static void RegisterFunction(string functionName, object target, MethodInfo method) {
			if (environment.ContainsKey(new Language.Lua.LuaString(functionName))) {
				if (DialogueDebug.LogWarnings) Debug.LogWarning(string.Format("{0}: Can't register Lua function {1}. A function with that name is already registered.", new System.Object[] { DialogueDebug.Prefix, functionName }));
			} else {
