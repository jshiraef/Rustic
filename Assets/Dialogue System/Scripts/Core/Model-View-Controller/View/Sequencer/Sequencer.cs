	...
	public class Sequencer : MonoBehaviour {
		...
		private void ActivateCommand(string commandName, string endMessage, string[] args) {
			...
			if (string.IsNullOrEmpty(commandName)) {
				if (DialogueDebug.LogInfo) Debug.Log(string.Format("{0}: Sequencer received a blank string as a command name", new System.Object[] { DialogueDebug.Prefix }));
		...
		private DisplaySettings.SubtitleSettings.ContinueButtonMode savedContinueButtonMode = DisplaySettings.SubtitleSettings.ContinueButtonMode.Always;
		...
		private bool HandleSetContinueModeInternally(string commandName, string[] args) {
			if (args == null || args.Length < 1) {
				if (DialogueDebug.LogWarnings) Debug.LogWarning(string.Format("{0}: Sequencer: SetContinueMode(true|false) requires a true/false parameter", new System.Object[] { DialogueDebug.Prefix }));
				return true;
			} else {
