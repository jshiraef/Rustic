...
namespace PixelCrushers.DialogueSystem {
	...
	public class ConversationView : MonoBehaviour {
		...
		private void PlayResponseMenuSequence(string responseMenuSequence) {
			if (string.IsNullOrEmpty(responseMenuSequence) && !string.IsNullOrEmpty(settings.cameraSettings.defaultResponseMenuSequence)) {
				responseMenuSequence = settings.cameraSettings.defaultResponseMenuSequence;
			}
		...
		public string GetDefaultSequence(Subtitle subtitle) {
			...
			if (isPlayerLine && !settings.subtitleSettings.showPCSubtitlesDuringLine) {
				return settings.cameraSettings.defaultPlayerSequence;
			} else {
				float duration = GetDefaultSubtitleDuration(subtitle.formattedText.text);
				var line = settings.cameraSettings.defaultSequence;
				if (isPlayerLine && !string.IsNullOrEmpty(settings.cameraSettings.defaultPlayerSequence)) {
					line = settings.cameraSettings.defaultPlayerSequence;
				}
