				...
				foreach (var entry in conversation.dialogueEntries) {
					foreach (var field in entry.fields) {
						if ((field.type == FieldType.Localization) &&
						    !otherDialogueText.Contains(field.title) &&
							!field.title.Contains(" ")) {
								otherDialogueText.Add(field.title);
						}
					}
				}
			}
			...
			foreach (var conversation in database.conversations) {
				...
				foreach (var entry in conversation.dialogueEntries) {
					if (entry.id > 0) {
						...
						sb.AppendFormat("{0},{1},{2}", entrytag, actorName, lineText);
						foreach (var fieldName in otherDialogueText) {
							sb.AppendFormat(",{0}", CleanField(Field.LookupValue(entry.fields, fieldName)));
						}
						file.WriteLine(sb.ToString());
					}
				}
