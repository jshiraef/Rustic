...
namespace PixelCrushers.DialogueSystem.DialogueEditor {
	...
	public partial class DialogueEditorWindow {
		...
		private void SetConversationDropdownIndex(int index) {
			...
			if (index > -1) {
				EditorPrefs.SetInt(ConversationIndexKey, index);
			}
		}
		...
		private void ActivateOutlineMode() {
			SetShowNodeEditor(false); //showNodeEditor = false;
		}
		
		private void ActivateNodeEditorMode() {
			SetShowNodeEditor(true); //showNodeEditor = true;
		...
		private void ResetNodeEditorConversationList() {
			...
			SetConversationDropdownIndex(GetCurrentConversationIndex()); //conversationIndex = GetCurrentConversationIndex();
		}
		...
		private void DrawNodeEditorConversationPopup() {
			...
			if (newIndex != conversationIndex) {
				SetConversationDropdownIndex(newIndex); //conversationIndex = newIndex;
