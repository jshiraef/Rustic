...
namespace PixelCrushers.DialogueSystem.DialogueEditor {
	...
	public partial class DialogueEditorWindow {
		...
		private void DrawQuestProperties(Item item) {
			...
			if (Field.FieldExists(item.fields, "Group")) {
			...
			if (trackable) {
				...
				bool newTrack = EditorGUILayout.Toggle(new GUIContent("Track on Start", "Tick to show in HUD when game starts."), track);
				if (newTrack != track) Field.SetValue(item.fields, "Track", newTrack);
			}
