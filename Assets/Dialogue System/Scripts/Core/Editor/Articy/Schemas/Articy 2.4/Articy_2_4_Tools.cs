...
namespace PixelCrushers.DialogueSystem.Articy.Articy_2_4 {
	...
	/// <summary>
	/// This static utility class contains tools to convert an articy:draft 2.4 XML schema into
	/// a schema-independent ArticyData object.
	/// </summary>
	public static class Articy_2_4_Tools {

		public static bool IsSchema(string xmlFilename) {
			return ArticyTools.ContainsSchemaId(xmlFilename, "http://www.nevigo.com/schemas/articydraft/2.4/XmlContentExport_FullProject.xsd");
		}
		
		public static ArticyData LoadArticyData(string xmlFilename, Encoding encoding) {
			return ConvertExportToArticyData(LoadExportFromXmlFile(xmlFilename, encoding));
		}

		//public static ExportType LoadExportFromXmlFile(string xmlFilename) {
		//	XmlSerializer xmlSerializer = new XmlSerializer(typeof(ExportType));
		//	return xmlSerializer.Deserialize(new StreamReader(xmlFilename)) as ExportType;
		//}
		...
		private static void ConvertItem(object item, List<Field> fields) {
			...
			if (type == typeof(BooleanPropertyType)) {
				...
				if (PixelCrushers.DialogueSystem.Articy.ArticyTools.IsQuestStateArticyPropertyName(enumProperty.Name)) {
					fields.Add(new Field(enumProperty.Name, PixelCrushers.DialogueSystem.Articy.ArticyTools.EnumValueToQuestState(Tools.StringToInt(enumProperty.Value)), FieldType.Text));
				} else {
