...
namespace PixelCrushers.DialogueSystem.Articy {
	...
	public class ArticyConverter {
		...
		private List<string> flowStack = new List<string>();

		public class DeferredJumpRecord {
			public DialogueEntry jumpEntry;
			public ArticyData.Jump jump;
			public DeferredJumpRecord(DialogueEntry jumpEntry, ArticyData.Jump jump) {
				this.jumpEntry = jumpEntry;
				this.jump = jump;
			}
		}
		private List<DeferredJumpRecord> deferredJumps = new List<DeferredJumpRecord>();
		...
		private void ConvertEntities() {
			...
			//---Was: Find even if no portrait folder is set: if (!string.IsNullOrEmpty(prefs.PortraitFolder)) 
		...
		private ArticyData.FlowFragment LookupFlowFragment(string id) {
		...
		private void ConvertJump(Conversation conversation, ArticyData.Jump jump) {
			if (jump != null) {
				...
				if (destination == null) {
					deferredJumps.Add(new DeferredJumpRecord(jumpEntry, jump));
				} else {
					Link link = new Link();
		...
		private void ConvertDeferredJumps() {
			foreach (var record in deferredJumps) {
				if (record == null) continue;
				var destination = FindDialogueEntryByArticyId(record.jump.target.idRef);
				if (destination == null) {
					if (record.jumpEntry != null) {
						Debug.LogWarning ("Dialogue System: Can't find destination for " + record.jumpEntry.Title);
					}
				} else {
					Link link = new Link();
					link.originConversationID = record.jumpEntry.conversationID;
					link.originDialogueID = record.jumpEntry.id;
					link.destinationConversationID = destination.conversationID;
					link.destinationDialogueID = destination.id;
					link.isConnector = false;
					link.priority = ConditionPriority.Normal;
					record.jumpEntry.outgoingLinks.Add(link);
				}
			}
		}
		...
		private void FindPortraitTexture(Actor actor) {
			...
			if (!string.IsNullOrEmpty(textureName)) {
				...
				string assetPath1 = string.Format("{0}/{1}", prefs.PortraitFolder, filename);
		...
		private void SortLinksByPosition(Conversation conversation) {
			...
			foreach (var entry in conversation.dialogueEntries) {
				entry.outgoingLinks.Sort(
					...
					if (destA == null || destB == null) Debug.Log ("destA=" + destA + ", destB=" + destB);
