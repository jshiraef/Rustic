...
namespace PixelCrushers.DialogueSystem {
	...
	public static class QuestLog {
		...
		/// <param name="questTitle">Quest title.</param>
		public static string GetQuestGroup(string questTitle) {
			return DialogueLua.GetLocalizedQuestField(questTitle, "Group").AsString;
		}
		...
		public static string[] GetAllGroups(QuestState flags, bool sortByGroupName) {
			...
			foreach (var itemTableValue in itemTable.Values) {
				...
				if (!isItem) {
					if (!groups.Contains(group) && IsQuestInStateMask(title, flags)) {
						groups.Add(group);
					}
				}
			}
		...
		/// </summary>
		/// <returns>
		/// The titles of all active quests, sorted by Name.
		/// </returns>
		/// <example>
			...
			foreach (var itemTableValue in itemTable.Values) {
				...
				if (!isItem) {
					if (string.IsNullOrEmpty(title)) {
						...
						if (IsQuestInStateMask(title, flags)) {
							titles.Add(title);
						}
					}
				}
		...
		/// <param name="flags">A bitmask of QuestState values.</param>
		/// <param name="flags">Sort by group and title.</param>
			...
			foreach (var itemTableValue in itemTable.Values) {
				...
				if (!isItem) {
					if (string.IsNullOrEmpty(title)) {
						...
						list.Add(new QuestGroupRecord(group, title));
					}
				}
			}
			if (sort) list.Sort();
