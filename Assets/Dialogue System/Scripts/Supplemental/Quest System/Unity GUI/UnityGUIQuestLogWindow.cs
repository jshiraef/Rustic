...
namespace PixelCrushers.DialogueSystem.UnityGUI {
	...
	/// </summary>
	[AddComponentMenu("Dialogue System/UI/Unity GUI/Quest/Unity GUI Quest Log Window")]
	public class UnityGUIQuestLogWindow : QuestLogWindow {
		...
		private GUIStyle GetGroupHeadingStyle() {
			return UseGUIStyle(groupHeadingStyle, groupHeadingGuiStyleName, GUI.skin.label);
		}
				...
				foreach (var questInfo in Quests) {
					...
					if (!string.Equals(questInfo.Group, currentGroup)) {
						...
						if (!string.IsNullOrEmpty(currentGroup)) {
							...
							GUI.Label(new Rect(padding, contentY, scrollView.contentWidth - (2 * padding), groupHeadingHeight), currentGroup, GetGroupHeadingStyle());
