	...
	public class PersistentPositionData : MonoBehaviour {
		...
		public void OnRecordPersistentData() {
			...
			if (recordCurrentLevel) {
				DialogueLua.SetActorField(actorName, "Position_" + Tools.loadedLevelName, positionString);
			} else {
		...
		public void OnApplyPersistentData() {
			...
			if (recordCurrentLevel) {
				if (restoreCurrentLevelPosition) {
					s = DialogueLua.GetActorField(actorName, "Position_" + Tools.loadedLevelName).AsString;
				}
			} else {
