		...
		private static void AppendConversationData(StringBuilder sb) {
			...
			try {
				...
				foreach (var conversation in DialogueManager.MasterDatabase.conversations) {
					//Debug.Log ("<color=cyan>Conv: " + conversation.id + "</color>");
					...
					for (int i = 0; i < conversation.dialogueEntries.Count; i++) {
						...
						if (dialogFields != null) {
							var simStatus = dialogFields.GetValue("SimStatus");
							//Debug.Log ("   " + entryID + ": " + simStatus);
							sb.AppendFormat("[{0}]={{SimStatus=\"{1}\"}},", new System.Object[] { entryID, simStatus });
						}
					}
					sb.Append("}; ");
				}
